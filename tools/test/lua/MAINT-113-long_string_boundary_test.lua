...
-- (T.text[path]). Both fixtures have NO trailing newline, which is the only shape that meets the
...
T.eq("B1 a --!text file ending in ']' with no trailing newline arrives whole",
    type(T.text) == "table" and T.text[A] or "(missing)", "a text that ends in a bracket]")
T.eq("B2 a --!text file containing ']]' and ending in ']=' arrives whole",
    type(T.text) == "table" and T.text[B] or "(missing)", "a text with a]] pair that ends in ]=")
