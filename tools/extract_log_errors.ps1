<#
.SYNOPSIS
    Extracts IncomeMod-related errors and warnings from FS25 game log.

.DESCRIPTION
    Parses the Farming Simulator 25 log.txt file and extracts all entries
    related to the IncomeMod mod, categorizing them by severity and type.
...
Write-Header "IncomeMod Log Analyzer"
...
# Patterns to match
$patterns = @{
    # IncomeMod specific patterns
    IncomeModLine = 'IncomeMod|FS25_IncomeMod'
    ModLoaded = 'v(\d+\.\d+\.\d+)'
...
foreach ($i in 0..($lines.Count - 1)) {
    ...
    # Check if line is IncomeMod related
    if ($line -match $patterns.IncomeModLine) {
        $stats.IncomeModLines++
    ...
    if ($line -match $patterns.XmlParse -and $line -match 'IncomeMod') {
        $stats.XmlErrors += @{
            Line = $i + 1
            Text = $line.Trim()
            Type = "XmlError"
...
$report += "IncomeMod-Related Lines: $($stats.IncomeModLines)"
